...
public class LuckyUtils {
    ...
    public static void setRequestedOrientation(Context context, int orientation) {
        if (LuckyUtils.getAppCompActivity(context) != null) {
            LuckyUtils.getAppCompActivity(context).setRequestedOrientation(
                    orientation);
        } else {
            LuckyUtils.scanForActivity(context).setRequestedOrientation(
                    orientation);
        }
    }
    ...
    public static Window getWindow(Context context) {
        if (LuckyUtils.getAppCompActivity(context) != null) {
            return LuckyUtils.getAppCompActivity(context).getWindow();
        } else {
            return LuckyUtils.scanForActivity(context).getWindow();
        }
    }
    ...
    public static void saveProgress(Context context, Object url, long progress) {
        if (!LuckyFrameLayout.SAVE_PROGRESS) return;
    ...
    public static long getSavedProgress(Context context, Object url) {
        if (!LuckyFrameLayout.SAVE_PROGRESS) return 0;
        SharedPreferences spn = context.getSharedPreferences("JZVD_PROGRESS",
                Context.MODE_PRIVATE);
